...
router.get('/detail/:proID', (req, res) => {
    ...
    productRepo.single(proID).then(product => {
        if (product) {
            ...
            console.log(productThumb.length);
            for(var i = 0; i <= productThumb.length; i++) {
                productThumbImage = productThumb[i];
            }
            Promise.all([brand, category, detail, product_same_brand, product_same_category, productThumb, productThumbImage]).then(([brand, category, detail, product_same_brand, product_same_category, productThumb, productThumbImage]) => {
                var vm = {
                    product: product,
                    category: category,
                    brand: brand,
